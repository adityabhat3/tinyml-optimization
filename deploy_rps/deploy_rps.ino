...
void tflu_initialization() {
  ...
  tflu_interpreter = new tflite::MicroInterpreter(tflu_model, tflu_ops_resolver, tensor_arena, tensor_arena_size, nullptr, nullptr); //change &tflu_error
  ...
  for (int yo = 0; yo < h1; yo++) {
    ...
    for(int xo = 0; xo < w1; xo++) {
      ...
      for(int i = 0; i < 3; i++) {
        ...
        c_f = rescale((float)c_i, 1.f/255.f, 0.f);
